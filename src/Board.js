...
export function Board(props) {
  ...
  function gameStatus(index) {
    ...
    const verticalCheck =
      board[index] + board[(index + 3) % 9] + board[(index + 6) % 9];
    ...
    for (let i = 0; i < 3; i++) {
      if (horizontalRanges[i].includes(index)) {
        horizontalCheck =
          board[horizontalRanges[i][0]] +
          board[horizontalRanges[i][1]] +
  ...
  useEffect(() => {
    ...
    } else if (gameStatus(lastIndex) == "draw") props.socket.emit("draw");
  }, [board]);
  ...
  useEffect(() => {
    ...
    props.socket.on("playAgain", (userType) => {
      if (userType === "X") setPlayAgainCheck((prev) => ["ready", prev[1]]);
      if (userType === "O") setPlayAgainCheck((prev) => [prev[0], "ready"]);
    });
  }, []);
